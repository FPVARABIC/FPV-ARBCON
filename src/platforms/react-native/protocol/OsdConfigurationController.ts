...
export type OsdBlockReason = 'DISCONNECTED' | 'IDENTIFYING' | 'UNSUPPORTED_FIRMWARE' | 'APP_BACKGROUNDED' | 'LINK_RECOVERING' | 'FC_ARMED' | 'ARMED_STATE_UNKNOWN' | 'MOTOR_TEST_ACTIVE' | 'CONFIGURATION_BUSY' | 'STALE_BASE' | 'INVALID_CONFIGURATION';
...
interface AmbiguousOsdCause {readonly kind: 'OSD_AMBIGUOUS_WRITE'; readonly stage: OsdSaveStage}
class AmbiguousOsdWriteError extends MspOperationOutcomeUnknownError {constructor(error: unknown, stage: OsdSaveStage) {super(Object.freeze({kind: 'OSD_AMBIGUOUS_WRITE', stage, error}));}}
...
export class OsdConfigurationController {
    ...
    try {
      ...
      const result = await this.operations(key.sessionId, client, scheduler).execute<{snapshot?: MspOsdSnapshot; readbackError?: unknown}>({
        ...
        execute: async requester => {
          ...
          for (const write of encodeChangedOsdConfiguration(original, draft)) await this.writeOnce(requester, MSP_SET_OSD_CONFIG, write.payload, {group: write.group, index: write.index});
  ...
  private async writeOnce(requester: MspRequester, command: number, payload: Uint8Array, stage: OsdSaveStage): Promise<void> {try {await requester.request(command, payload, {wireFormat: 'v1'});} catch (error) {const code = errorCode(error); if (code !== undefined && DEFINITELY_NOT_SENT.has(code)) throw error; throw new AmbiguousOsdWriteError(error, stage);}}
  private capture(key: SetupUiSessionKey): {client: OsdClient; scheduler: MspTelemetryScheduler; epoch: number} | {reason: OsdBlockReason} {if (this.appStateOwner.getPhase() !== 'ACTIVE') return {reason: 'APP_BACKGROUNDED'}; if (this.isMotorTestActive(key.sessionId)) return {reason: 'MOTOR_TEST_ACTIVE'}; const identification = this.coordinator.getIdentificationState(key.sessionId); if (identification.status === 'IDLE' || identification.status === 'RUNNING') return {reason: 'IDENTIFYING'}; if (identification.status !== 'SUCCEEDED' || identification.identity.firmware.identifier !== 'BTFL' || identification.identity.apiVersion.apiVersionMajor !== 1 || identification.identity.apiVersion.apiVersionMinor !== 47) return {reason: 'UNSUPPORTED_FIRMWARE'}; const client = this.coordinator.getActiveMspClient(key.sessionId); const scheduler = this.coordinator.getTelemetryScheduler(key.sessionId); if (this.coordinator.getOwnershipState(key.sessionId) !== 'ACTIVE' || this.coordinator.getSessionKey(key.sessionId)?.generation !== key.generation || client === undefined || scheduler === undefined) return {reason: 'DISCONNECTED'}; if (this.coordinator.getMspRecoveryState(key.sessionId) !== 'READY') return {reason: 'LINK_RECOVERING'}; return {client, scheduler, epoch: client.getEpoch()};}
