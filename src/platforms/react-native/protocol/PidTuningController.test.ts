...
import {MSP_BOXIDS, MSP_EEPROM_WRITE, MSP_FILTER_CONFIG, MSP_PID, MSP_PID_ADVANCED, MSP_RC_TUNING, MSP_SET_PID, MSP_SET_PID_ADVANCED, MSP_STATUS_EX} from '../../../core/protocol/msp/commands/mspCommands';
...
function identification(identifier = 'BTFL', minor = 47): MspIdentificationState { return {status: 'SUCCEEDED', identity: {firmware: {identifier, knownFamily: identifier === 'BTFL' ? 'BETAFLIGHT' : 'INAV'}, apiVersion: {mspProtocolVersion: 0, apiVersionMajor: 1, apiVersionMinor: minor}, board: {}}} as MspIdentificationState; }
...
function enqueueSnapshot(client: FakeClient, rollP = 42, yawF = 140) { client.enqueue(MSP_PID, {payload: pidPayload(rollP)}); client.enqueue(MSP_PID_ADVANCED, {payload: advancedPayload(yawF)}); client.enqueue(MSP_RC_TUNING, {payload: new Uint8Array(24)}); client.enqueue(MSP_FILTER_CONFIG, {payload: new Uint8Array(49)}); }
