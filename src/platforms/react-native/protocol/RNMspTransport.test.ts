...
function makeFakeClient(): FakeClient {
  ...
  const onSessionDetached = jest.fn(
...
function makeTransport(client: FakeClient = makeFakeClient()) {
  const transport = new RNMspTransport(
...
describe('base64 round-trip (base64.ts)', () => {
  ...
  it('decodes a large document payload incrementally without changing bytes', async () => {
    ...
    await expect(
      base64ToBytesAsync(bytesToBase64(bytes), 32 * 1024),
...
describe('RNMspTransport.writeBytes', () => {
  ...
  it('rejects with {code, message} (nativeMessage renamed to message; code untouched) on a native rejection', async () => {
    const { client, transport } = makeTransport();
    client.writeBytes.mockRejectedValueOnce({
      code: 'WRITE_FAILED',
      nativeMessage: 'native boom',
    });
...
describe('RNMspTransport.onDataReceived - session filtering and ordering', () => {
  it('filters events for a different sessionId BEFORE any decoding is attempted', () => {
    ...
    client.emitData({
      ...
      dataBase64: bytesToBase64(Uint8Array.from([9])),
    });
  ...
  it('delivers multiple data events for this session in exact delivery order', () => {
    ...
    client.emitData({
      ...
      dataBase64: bytesToBase64(Uint8Array.from([1])),
    });
    client.emitData({
      sessionId: SESSION_ID,
      dataBase64: bytesToBase64(Uint8Array.from([2])),
    });
    client.emitData({
      sessionId: SESSION_ID,
      dataBase64: bytesToBase64(Uint8Array.from([3])),
    });
  ...
  it('a throwing FIRST listener does not prevent the SECOND or THIRD from running, and the exception never escapes into the real client emit call', () => {
    ...
    expect(() =>
      client.emitData({
        sessionId: SESSION_ID,
        dataBase64: bytesToBase64(Uint8Array.from([7])),
      }),
    ).not.toThrow();
...
describe('RNMspTransport raw mode', () => {
  it('diverts CLI bytes away from MSP listeners and restores MSP after release', () => {
    const { client, transport } = makeTransport();
    const msp: number[][] = [];
    const raw: number[][] = [];
    transport.onDataReceived(bytes => msp.push([...bytes]));
    const release = transport.enterRawMode(bytes => raw.push([...bytes]));

    client.emitData({
      sessionId: SESSION_ID,
      dataBase64: bytesToBase64(Uint8Array.from([35, 32])),
    });
    expect(raw).toEqual([[35, 32]]);
    expect(msp).toEqual([]);
    expect(() => transport.enterRawMode(() => undefined)).toThrow(
      /already active/,
    );
    ...
    client.emitData({
      sessionId: SESSION_ID,
      dataBase64: bytesToBase64(Uint8Array.from([36, 77])),
    });
    expect(msp).toEqual([[36, 77]]);
...
describe('RNMspTransport.onSessionDetached - session filtering', () => {
  ...
  it('a throwing FIRST listener does not prevent the SECOND or THIRD from running, and the exception never escapes into the real client emit call', () => {
    ...
    transport.onSessionDetached(() => {
      throw new Error(
        'boom - simulated throwing first session-detached listener',
      );
    });
...
describe('RNMspTransport.restartReceiveLoop', () => {
  ...
  it('never calls startReading() if stopReading() rejects, and rejects with stopReading()s reason ({code, message})', async () => {
    const { client, transport } = makeTransport();
    client.stopReading.mockRejectedValueOnce({
      code: 'RX_RESTART_TIMEOUT',
      nativeMessage: 'timed out',
    });
  ...
  it('propagates a startReading() rejection (e.g. RX_ALREADY_ACTIVE) with code unchanged and message renamed', async () => {
    ...
    client.startReading.mockRejectedValueOnce({
      code: 'RX_ALREADY_ACTIVE',
      nativeMessage: 'already active',
    });
...
describe('RNMspTransport.dispose', () => {
  ...
  it('writeBytes() and restartReceiveLoop() called after dispose() reject immediately without reaching the real client', async () => {
    ...
    await expect(transport.writeBytes(new Uint8Array(0))).rejects.toMatchObject(
      { code: 'SESSION_CLOSED' },
    );
    await expect(transport.restartReceiveLoop()).rejects.toMatchObject({
      code: 'SESSION_CLOSED',
    });
...
describe('RNMspTransport - multiple independent listeners on ONE real subscription', () => {
  it('two listeners both receive the same data events, and one unsubscribing does not affect the other or the real subscription', () => {
    ...
    const unsubscribeSecond = transport.onDataReceived(bytes =>
      secondReceived.push(Array.from(bytes)),
    );
  ...
  it('the same independence holds for onSessionDetached listeners', () => {
    ...
    const unsubscribeSecond = transport.onSessionDetached(event =>
      second.push(event),
    );
