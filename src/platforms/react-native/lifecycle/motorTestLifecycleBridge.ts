...
/**
 * B-2E-1: the bridge binds to the coordinator-owned controller's SEALED
 * LIFECYCLE STOP PORT, never to a controller.
 *
 * Two members and nothing else: it can read, and it can request a
 * whitelisted stop. It structurally CANNOT initialize a session, close
 * one, construct a controller, or reach a client, lease or writer - so no
 * lifecycle listener can ever be the thing that starts a motor-test
 * session or a motor.
 */
export interface MotorTestBridgeStopPort {
...
export interface MotorTestLifecycleBridgeOptions {
  readonly controller: MotorTestBridgeStopPort;
...
class LifecycleBridge implements MotorTestLifecycleBridge {
  private readonly controller: MotorTestBridgeStopPort;
