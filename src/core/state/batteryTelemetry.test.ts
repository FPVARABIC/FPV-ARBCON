...
describe('deriveBatterySemantics', () => {
  it('reports NOT_DETECTED exactly when cellCount is 0 (the verified firmware meaning) - and DETECTED otherwise', () => {
    expect(deriveBatterySemantics(raw({cellCount: 0})).detection).toBe('NOT_DETECTED');
    expect(deriveBatterySemantics(raw({cellCount: 1})).detection).toBe('DETECTED');
    expect(deriveBatterySemantics(raw({cellCount: 6})).detection).toBe('DETECTED');
  ...
  it('maps every verified batteryState_e value to its named firmware state', () => {
    expect(deriveBatterySemantics(raw({batteryStateRaw: 0})).firmwareState).toBe('OK');
    expect(deriveBatterySemantics(raw({batteryStateRaw: 1})).firmwareState).toBe('WARNING');
    expect(deriveBatterySemantics(raw({batteryStateRaw: 2})).firmwareState).toBe('CRITICAL');
    expect(deriveBatterySemantics(raw({batteryStateRaw: 3})).firmwareState).toBe('NOT_PRESENT');
    expect(deriveBatterySemantics(raw({batteryStateRaw: 4})).firmwareState).toBe('INIT');
  });

  it('preserves an unknown enum value as {kind:UNKNOWN, raw} and NEVER defaults it to OK', () => {
    const state = deriveBatterySemantics(raw({batteryStateRaw: 9})).firmwareState;
  ...
  it('uses the high-resolution centivolt field as the canonical voltage and keeps the saturated legacy field as a separate echo', () => {
    const semantics = deriveBatterySemantics(raw({voltageCentivolts: 2570, legacyVoltageDecivolts: 255}));
    expect(semantics.voltageVolts).toBeCloseTo(25.7, 10);
    expect(semantics.legacyVoltageVolts).toBeCloseTo(25.5, 10);
  });

  it('marks current and consumed mAh as sensorValidity UNPROVEN while preserving their raw values - including zero and negative', () => {
    const zero = deriveBatterySemantics(raw({amperageCentiamps: 0, consumedMah: 0}));
    ...
    expect(negative.current).toEqual({centiamps: -250, sensorValidity: 'UNPROVEN'});
  });

  it('passes configured capacity through as configuration - zero stays a configured zero, never an availability judgment', () => {
    expect(deriveBatterySemantics(raw({configuredCapacityMah: 0})).configuredCapacityMah).toBe(0);
    expect(deriveBatterySemantics(raw({configuredCapacityMah: 1500})).configuredCapacityMah).toBe(1500);
  });
