...
describe('OSD API 1.47 decoding', () => {
  ...
  it('requires a valid exact canvas payload', () => {expect(decodeOsdCanvas(Uint8Array.from([53, 20]))).toEqual({columns: 53, rows: 20}); expect(() => decodeOsdCanvas(Uint8Array.from([53]))).toThrow(); expect(() => decodeOsdCanvas(Uint8Array.from([0, 20]))).toThrow();});
});
