...
function render(
  ...
  options: {connected?: boolean; channelState?: AuxTelemetryChannelState; gpsPresent?: boolean | undefined} = {},
...
describe('GpsCard', () => {
  it('disconnected renders the approved copy before anything else', () => {
    const renderer = render({status: 'FRESH', value: FIX, updatedAtMs: 0}, {connected: false});
    expect(allText(renderer)).toContain('غير متصل');
    expect(renderer.root.findAllByProps({testID: 'gps-card-disconnected'}).length).toBeGreaterThan(0);
    unmount(renderer);
  });
  ...
  it('an UNSUPPORTED channel verdict renders the approved "غير مدعوم" copy (the FC rejected MSP_RAW_GPS)', () => {
    ...
    expect(renderer.root.findAllByProps({testID: 'gps-card-unsupported'}).length).toBeGreaterThan(0);
    unmount(renderer);
  });
  ...
  it('UNAVAILABLE / WAITING / ERROR values map to the shared approved copy', () => {
    const cases: Array<[TelemetryValue<MspRawGpsCompact>, string, string]> = [
      ...
      [{status: 'ERROR', error: new Error('boom')}, 'تعذرت قراءة البيانات', 'gps-card-error'],
    ];
  ...
  it('a fix renders the approved located copy with the satellite count', () => {
    ...
    expect(renderer.root.findAllByProps({testID: 'gps-card-fix'}).length).toBeGreaterThan(0);
    unmount(renderer);
  });

  it('a fix is its own presence proof - located copy renders even when the STATUS_EX bit is unavailable', () => {
    const renderer = render({status: 'FRESH', value: FIX, updatedAtMs: 0}, {gpsPresent: undefined});
  ...
  it('no fix with PROVEN presence renders the approved "not yet located" copy (not an error) with the count', () => {
    const renderer = render({status: 'FRESH', value: NO_FIX, updatedAtMs: 0}, {gpsPresent: true});
    const text = allText(renderer);
  ...
  it('no fix WITHOUT presence proof (false or unprovable) renders the honest unavailable copy - zero satellites never proves absence', () => {
    for (const gpsPresent of [false, undefined]) {
      const renderer = render({status: 'FRESH', value: {hasFix: false, satelliteCount: 0}, updatedAtMs: 0}, {gpsPresent});
      expect(allText(renderer)).toContain('البيانات غير متاحة');
      expect(renderer.root.findAllByProps({testID: 'gps-card-no-presence'}).length).toBeGreaterThan(0);
      unmount(renderer);
    }
  });
  ...
  it('STALE freezes the content dimmed with the approved stale label', () => {
    const renderer = render({status: 'STALE', value: FIX, updatedAtMs: 0, ageMs: 15500});
    const text = allText(renderer);
    expect(text).toContain('تم تحديد الموقع');
    expect(text).toContain('القراءة غير محدثة');
    expect(renderer.root.findAllByProps({testID: 'gps-card-stale'}).length).toBeGreaterThan(0);
    unmount(renderer);
  });
  ...
  it('carries a complete Arabic accessibility label including fix state and satellite count', () => {
    ...
    expect(card.props.accessibilityLabel).toBe('GPS، تم تحديد الموقع، عدد الأقمار: 11');
    unmount(renderer);
  });
