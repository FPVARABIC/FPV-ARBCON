...
import type {MspStatusExDiagnostics, SetupDiagnosticsInput, SetupDiagnosticsView} from '../../../core';
...
function view(overrides: Partial<SetupDiagnosticsInput> = {}): SetupDiagnosticsView {
...
function render(diagnostics: SetupDiagnosticsView): ReactTestRenderer.ReactTestRenderer {
  let renderer!: ReactTestRenderer.ReactTestRenderer;
  act(() => {
    renderer = ReactTestRenderer.create(<DiagnosticsSection view={diagnostics} />);
  });
  return renderer;
}
...
function host(renderer: ReactTestRenderer.ReactTestRenderer, testID: string): ReactTestRenderer.ReactTestInstance {
  return renderer.root.find(node => typeof node.type === 'string' && node.props.testID === testID);
}

function label(renderer: ReactTestRenderer.ReactTestRenderer, testID: string): string {
...
describe('DiagnosticsSection - Region 4 structure', () => {
  ...
  it('keeps a stable reading order: identity, compatibility, reading state, sensors, blockers', () => {
    ...
    const order = renderer.root
      ...
      .filter((id: unknown): id is string => typeof id === 'string' && id.startsWith('diagnostics-'));
...
describe('DiagnosticsSection - identity and compatibility copy', () => {
  it('shows the board and the raw firmware identifier with the reported API version', () => {
    ...
    expect(texts(renderer)).toEqual(
      expect.arrayContaining(['اللوحة: SPEEDYBEEF405V3', 'البرنامج الثابت: BTFL — واجهة MSP 1.47']),
    );
    expect(texts(renderer)).toContain('متوافق: Betaflight بواجهة MSP 1.47');
    unmount(renderer);
  });
  ...
  it('does not claim compatibility for another API version', () => {
    ...
    expect(texts(renderer)).toContain('واجهة غير مُتحقَّق منها في هذا الإصدار؛ تُعرض القراءات فقط');
    expect(texts(renderer)).not.toContain('متوافق: Betaflight بواجهة MSP 1.47');
    unmount(renderer);
  });

  it('states identification progress and failure without inventing a board', () => {
    const identifying = render(view({identificationStatus: 'RUNNING', identity: undefined}));
    expect(texts(identifying)).toContain('جارٍ التعرّف على متحكم الطيران');
    unmount(identifying);

    const failed = render(view({identificationStatus: 'FAILED', identity: undefined}));
...
describe('DiagnosticsSection - lifecycle state copy', () => {
  const cases: Array<[string, Partial<SetupDiagnosticsInput>, string]> = [
    ...
    ['unavailable', {status: 'UNAVAILABLE', value: undefined}, 'البيانات غير متاحة'],
    ['waiting', {status: 'WAITING', value: undefined}, 'بانتظار أول قراءة'],
    ['read error', {status: 'ERROR', value: undefined}, 'تعذرت قراءة البيانات'],
    ['stale', {status: 'STALE'}, 'القراءة غير محدثة'],
    ['fresh', {}, 'القراءة محدثة'],
  ];

  it.each(cases)('%s renders its own explicit text', (_name, overrides, expected) => {
  ...
  it('dims sensor and blocker blocks while stale, but never the identity block', () => {
    ...
    const opacityOf = (testID: string): number | undefined => {
      const styles = host(renderer, testID).props.style as Array<{opacity?: number} | undefined>;
...
describe('DiagnosticsSection - sensor truthfulness', () => {
  it('lists only the detected sensors of the pinned mapping, under a "reported as detected" heading', () => {
    const renderer = render(view());
    expect(texts(renderer)).toEqual(expect.arrayContaining(['المستشعرات المُبلَّغ عن اكتشافها', 'ACC', 'GPS', 'GYRO']));
    expect(texts(renderer)).not.toContain('BARO');
    unmount(renderer);
  });

  it('preserves an unknown sensor bit in hex instead of dropping or naming it', () => {
    const renderer = render(view({value: value({sensorPresenceMask: Math.pow(2, 9)})}));
  ...
  it('an empty mask states the reading, never a health conclusion', () => {
    ...
    expect(texts(renderer)).toContain('لم يُبلِّغ متحكم الطيران عن أي مستشعر في هذه القراءة');
    unmount(renderer);
  });

  it('says sensors cannot be confirmed when there is no reading', () => {
    const renderer = render(view({status: 'WAITING', value: undefined}));
    expect(texts(renderer)).toContain('لا يمكن تأكيد المستشعرات في الوقت الحالي');
    unmount(renderer);
  });
});
...
describe('DiagnosticsSection - blocker truthfulness', () => {
  ...
  it('renders a source-proven Arabic description alongside the canonical token', () => {
    const renderer = render(view({value: value({readiness: {armingDisableFlags: Math.pow(2, 7)}})}));
  ...
  it('renders every known blocker bit with a description and its token', () => {
    for (let bit = 0; bit < 29; bit++) {
      const renderer = render(view({value: value({readiness: {armingDisableFlags: Math.pow(2, bit)}})}));
      const lines = texts(renderer);
      const blockerLine = host(renderer, 'diagnostics-blockers-line-0').props.children as string;
  ...
  it('lists multiple simultaneous blockers, one line each', () => {
    ...
    const renderer = render(view({value: value({readiness: {armingDisableFlags: mask}})}));
    expect(texts(renderer)).toEqual(
      expect.arrayContaining(['لم يتم اكتشاف الجيروسكوب (NO_GYRO)', 'الطائرة ليست مستوية (ANGLE)']),
    );
    unmount(renderer);
  });

  it('preserves an unknown blocker bit numerically AND in hex', () => {
    const renderer = render(view({value: value({readiness: {armingDisableFlags: Math.pow(2, 30)}})}));
