...
function NumericField({label, value, min = 0, max, disabled, onChange, testID}: {label: string; value: number; min?: number; max: number; disabled: boolean; onChange: (value: number) => void; testID: string}) {
  const apply = (next: number) => onChange(Math.max(min, Math.min(max, next)));
...
function ScaledNumericField({label, rawValue, rawMin, rawMax, scale, disabled, onChange, testID}: {label: string; rawValue: number; rawMin: number; rawMax: number; scale: number; disabled: boolean; onChange: (value: number) => void; testID: string}) {
  ...
  const apply = (raw: number) => onChange(Math.max(rawMin, Math.min(rawMax, Math.round(raw))));
  return <View style={styles.numericField}><Text style={styles.fieldLabel}>{label}</Text><View style={styles.stepper}><Pressable disabled={disabled} onPress={() => apply(rawValue - 1)} style={styles.stepButton}><Text style={styles.stepText}>−</Text></Pressable><TextInput value={(rawValue * scale).toFixed(decimals)} editable={!disabled} keyboardType="decimal-pad" selectTextOnFocus onChangeText={text => { const parsed = Number.parseFloat(text.replace(',', '.')); if (Number.isFinite(parsed)) apply(parsed / scale); }} style={styles.numberInput} testID={testID} /><Pressable disabled={disabled} onPress={() => apply(rawValue + 1)} style={styles.stepButton}><Text style={styles.stepText}>+</Text></Pressable></View><Text style={styles.rangeHint}>{(rawMin * scale).toFixed(decimals)}–{(rawMax * scale).toFixed(decimals)}</Text></View>;
}
...
const styles = StyleSheet.create({
  root: {flex: 1, backgroundColor: colors.background}, content: {width: '100%', alignSelf: 'center', padding: spacing.lg, gap: spacing.md}, hero: {flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: spacing.lg, flexWrap: 'wrap'}, heroCopy: {flex: 1, minWidth: 280, gap: 4}, eyebrow: {...typography.eyebrow, color: colors.accentStrong, letterSpacing: 1}, title: {...typography.title, color: colors.textPrimary, textAlign: 'right'}, subtitle: {...typography.body, color: colors.textSecondary, textAlign: 'right', writingDirection: 'rtl'}, profileBadges: {flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm}, profileBadge: {borderWidth: 1, borderColor: colors.accentStrong, backgroundColor: colors.accentSoft, borderRadius: radii.lg, padding: spacing.md, minWidth: 150}, profileLabel: {...typography.caption, color: colors.textMuted, textAlign: 'right'}, profileValue: {...typography.heading, color: colors.accentStrong, textAlign: 'right'}, danger: {borderRadius: radii.lg, borderWidth: 1, borderColor: colors.error, backgroundColor: '#FFF0F2', padding: spacing.md, gap: 3}, dangerTitle: {...typography.heading, color: colors.error, textAlign: 'right'}, dangerText: {...typography.body, color: colors.error, textAlign: 'right', writingDirection: 'rtl'}, hardwareNotice: {borderRadius: radii.lg, borderWidth: 1, borderColor: colors.accentStrong, backgroundColor: colors.accentSoft, padding: spacing.md, gap: 3}, hardwareTitle: {...typography.eyebrow, color: colors.accentStrong}, hardwareText: {...typography.caption, color: colors.accentText, textAlign: 'right', writingDirection: 'rtl'}, sectionHeading: {gap: 3}, sectionTitle: {...typography.heading, color: colors.textPrimary, textAlign: 'right'}, sectionHint: {...typography.caption, color: colors.textMuted, textAlign: 'right', writingDirection: 'rtl'}, axisGrid: {gap: spacing.md}, axisGridWide: {flexDirection: 'row'}, axisCard: {flex: 1, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.borderSoft, borderRadius: radii.lg, padding: spacing.lg, gap: spacing.md}, axisTitle: {...typography.title, fontSize: 24, color: colors.accentStrong, textAlign: 'right'}, axisSubtitle: {...typography.caption, color: colors.textMuted, textAlign: 'right'}, fieldsRow: {flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm}, numericField: {flexGrow: 1, flexBasis: 100, gap: 5}, fieldLabel: {...typography.heading, color: colors.textPrimary, textAlign: 'center'}, stepper: {flexDirection: 'row', alignItems: 'stretch', minHeight: 44}, stepButton: {width: 36, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.border, backgroundColor: colors.backgroundRaised}, stepText: {fontSize: 20, color: colors.accentStrong, fontWeight: '700'}, numberInput: {flex: 1, minWidth: 52, borderTopWidth: 1, borderBottomWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, color: colors.textPrimary, textAlign: 'center', fontVariant: ['tabular-nums'], fontWeight: '700'}, rangeHint: {...typography.caption, color: colors.textMuted, textAlign: 'center'}, readOnlyGrid: {gap: spacing.md}, readOnlyGridWide: {flexDirection: 'row'}, card: {flex: 1, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.borderSoft, borderRadius: radii.lg, padding: spacing.lg, gap: spacing.sm}, readout: {...typography.body, color: colors.textSecondary, textAlign: 'right', fontVariant: ['tabular-nums']}, choiceRow: {flexDirection: 'row', gap: spacing.sm, flexWrap: 'wrap'}, choice: {minHeight: 42, minWidth: 88, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.border, borderRadius: radii.md, backgroundColor: colors.backgroundRaised, paddingHorizontal: spacing.md}, choiceSelected: {borderColor: colors.accentStrong, backgroundColor: colors.accentSoft}, choiceText: {...typography.body, color: colors.textSecondary, fontWeight: '700'}, choiceTextSelected: {color: colors.accentStrong}, rateEvidence: {flexDirection: 'row', justifyContent: 'flex-end', gap: spacing.lg, flexWrap: 'wrap', borderRadius: radii.md, borderWidth: 1, borderColor: colors.success, backgroundColor: '#E8F8F1', padding: spacing.sm}, warning: {borderRadius: radii.md, borderWidth: 1, borderColor: colors.warning, backgroundColor: '#FFF7E7', padding: spacing.md, gap: spacing.sm}, warningText: {...typography.body, color: colors.warning, textAlign: 'right', writingDirection: 'rtl'}, success: {borderRadius: radii.md, borderWidth: 1, borderColor: colors.success, backgroundColor: '#E8F8F1', padding: spacing.md}, successText: {...typography.body, color: colors.success, textAlign: 'right'}, inlineAction: {alignSelf: 'flex-start', minHeight: 40, justifyContent: 'center', paddingHorizontal: spacing.md, borderRadius: radii.md, backgroundColor: colors.warning}, inlineActionText: {...typography.body, color: colors.white, fontWeight: '700'}, loading: {...typography.body, color: colors.textSecondary, textAlign: 'center', padding: spacing.xl}, bottomSpace: {height: spacing.xl},
});
