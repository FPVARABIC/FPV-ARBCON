 ...
 */
/**
 * NO CONNECTION GATE LIVES HERE ANY MORE, and that is the point.
 *
 * An earlier attempt wrapped each panel in a gate that swapped the
 * screen for a "no flight controller connected" card. It worked, and it
 * was the wrong shape: the operator still got the shell, the tab bar and
 * fifteen reachable destinations, each answering that the app could not
 * actually do anything. The wall moved UP - this whole screen is
 * registered in the navigator only while a flight controller is verified
 * (App.tsx, ui/session/verifiedConnection.ts), so by the time any of
 * this mounts there is a live, current, identified board behind it.
 */
