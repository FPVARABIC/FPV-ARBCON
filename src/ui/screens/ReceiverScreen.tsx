...
const styles = StyleSheet.create({
  ...
  // P1 CLIPPING FIX. These two were `previewCard`/`channelsCard`, applied
  // unconditionally. `flex: N` is flex-grow with a flex-basis of 0, so in
  // the COLUMN layout (primaryGrid WITHOUT primaryGridWide, i.e. every
  // width below 900) the two cards were sized as a 4:5 split of the
  // container instead of by their own content. Measured in a browser at
  // 768 with 16 live channels: the channels card's box ended 90px above
  // its own last row, so AUX 10 was cut mid-row and AUX 11 and AUX 12
  // were painted over by the following card - unreachable at any scroll
  // position. Live data silently hidden on the screen that exists to show
  // it. The proportional split IS wanted in the side-by-side row layout,
  // so it is now applied only there; in column layout the cards size to
  // their content. Layout-only: no hierarchy, typography or component
  // change, which all remain P3.
  previewCardWide: {flex: 4}, channelsCardWide: {flex: 5},
  primaryGrid: {gap: spacing.md}, primaryGridWide: {flexDirection: 'row'}, card: {backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.borderSoft, borderRadius: radii.lg, padding: spacing.lg, gap: spacing.md}, sectionHeading: {gap: 3}, sectionTitle: {...typography.heading, color: colors.textPrimary, textAlign: 'right'}, sectionHint: {...typography.caption, color: colors.textMuted, textAlign: 'right', writingDirection: 'rtl'},
