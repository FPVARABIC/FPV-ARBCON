...
## الوضع القياسي مقابل «متقدم» (بعد إعادة التنظيم)

الشاشة الافتراضية للتفليش هي الوضع القياسي، والشاشة الكاملة القديمة تبقى خلف زر «متقدم». التقسيم أدناه هو عقد المنتج: القدرة الرسمية للبناء موجودة في الوضع القياسي، والأدوات المتخصصة فقط هي التي انتقلت إلى «متقدم».

| القدرة | Betaflight Configurator | الوضع القياسي | «متقدم» |
|---|---|---|---|
| اختيار Target من الكتالوج الرسمي كاملاً | نعم | نعم، بحث بالاسم/الشركة/MCU من `/api/targets` بلا أي قائمة بيضاء | نعم |
| التعرف التلقائي على اللوحة وربطها بـ Target | نعم | نعم، ومع هوية غير موجودة في الكتالوج يُعرض السبب ويبقى الاختيار اليدوي | نعم |
| قناة الإصدار Stable / RC / Development | نعم | Stable افتراضياً، والقنوات الأخرى خلف محدد ثانوي؛ لا استبدال صامت | نعم |
| Radio / Telemetry / OSD / Motor Protocol | نعم | نعم، تُبنى من `/api/options/{release}` فقط | نعم |
| خيارات أخرى (General Options) | نعم | نعم، اختيار متعدد بالقيم الرسمية | نعم |
| Custom Defines | نعم | نعم، مطوية داخل إعدادات البناء | نعم |
| Core Build | نعم | يُختار تلقائياً عندما لا يوفّر الإصدار خيارات | نعم، تبديل صريح |
| Commit تطويري محدد | نعم | لا (يبقى في «متقدم») | نعم |
| الدخول البرمجي إلى وضع DFU | نعم | نعم، زر «الدخول إلى وضع DFU»؛ وعند عدم الدعم تُعرض تعليمات يدوية قصيرة عند الحاجة فقط | نعم |
| إذن WebUSB لجهاز DFU | نعم | يُطلب مرة واحدة وعند الحاجة فقط | نعم |
| التحقق من مطابقة Target قبل التفليش | تحذير | حظر؛ ولا يوجد تجاوز عابر في الوضع القياسي | تجاوز خبير صريح |
| Full Chip Erase / No Reboot / Manual Baud / Read Unprotect | نعم | لا | نعم |
| ملف Firmware محلي / UF2 / ESP | نعم | لا | نعم |
| النسخ الاحتياطي والاستعادة عبر CLI | نعم | لا | نعم |
| نتيجة نهائية واحدة: نجاح / فشل / تعذّر التأكيد | جزئياً | نعم | نعم |

قدرات مرجعية غير معروضة في الوضع القياسي عمداً: اختيار Commit، الملفات المحلية، أدوات الاسترداد منخفضة المستوى، تجاوز عدم تطابق Target. جميعها موجودة في «متقدم» لأنها إما متخصصة أو تُضعف بوابة الأمان إذا صارت افتراضية.

خيارات مثل Position Hold: لا تُضاف يدوياً في أي مكان. إن أعادها `/api/options/{release}` ضمن `generalOptions` ظهرت تلقائياً بقيمتها الرسمية تحت «خيارات أخرى»، وإن لم يُعدها الإصدار المختار فلا تظهر.
