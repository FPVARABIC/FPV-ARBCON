...
| تنعيم الراديو (المعامل التلقائي) | **90** `[A]` (S1.1) | يذيب درجات إشارة الراديو إلى حركة متصلة | تريد نعومة أقصى → 180 (S1.2) |
| تنعيم الخانق | **50** `[A]` (S1.1) | يمنع قفزات الارتفاع الظاهرة في الصورة | — |
...
| Bidirectional DShot | **مفعّل** `[A]` (S1.11a) | شرط فلتر RPM، وهو ما يجعل الفيديو خاليًا من الاهتزاز | لا يدعمه ESC |
| عدد أقطاب المحرك | حسب محركك `[C]` | رقم خاطئ = فلتر يعمل على تردد غير موجود | — |
| Dynamic Idle | **35** (×100 rpm) `[A]` (S1.11a) | يمنع توقف المروحة في الهبوط الحاد | وووب 1S → 120–150 (S1.8) |

### اختياري

| اضبط | ابدأ بـ | لماذا | غيّرها إذا |
|---|---|---|---|
| دفعة الاستجابة (Boost) | **0** `[A]` | الإعدادات السينمائية الرسمية لا تضبطها | — |
...
| ما تضبطه | الشاشة |
|---|---|
| تجاهل ارتجاف العصا · التنعيم · دفعة الاستجابة | ضبط PID → بطاقة **إحساس العصا** |
| تنعيم الراديو | المستقبل → بطاقة التنعيم |
| Dynamic Idle · الفلاتر | ضبط PID |
| Bidirectional DShot · أقطاب المحرك | المحركات → الإعدادات |
| إجراء Failsafe | Failsafe |
| إنذار RSSI dBm | OSD → التنبيهات |

![بطاقة إحساس العصا في شاشة ضبط PID](../_shared/images/pid-feel-card.png)

![بطاقة تنعيم الراديو في شاشة المستقبل](../_shared/images/receiver-smoothing-card.png)
