...
const {mkdtempSync, readFileSync, readdirSync, rmSync, statSync} = require('fs');
...
function collectSourceFiles(directory, into) {
  for (const name of readdirSync(directory)) {
    ...
    if (path.includes(`${'__tests__'}`) || path.includes(`${'__testUtils__'}`)) {
      continue;
    }
    into.push(path);
  }
...
function analyzeEngineBoundaries(sources) {
  ...
  const dispatchSites = Array.from(
    executable.matchAll(/([A-Za-z_$][\w$]*)\s*\.\s*([A-Za-z_$][\w$]*)\s*\(\s*MSP_SET_MOTOR/g),
  ).map(match => ({receiver: match[1], method: match[2]}));
  const unleashed = dispatchSites.filter(
    site => site.receiver !== 'lease' && !LEASE_GUARDED_DISPATCH.includes(site.method),
  );
...
function main() {
  ...
  console.log(`A. Strictly forbidden (debug-only) tokens: ${FORBIDDEN_TOKENS.length} scanned`);
  ...
  for (const token of bundle.missingEngine) {
    console.error(
      `   MISSING ENGINE TOKEN: ${JSON.stringify(token)} - the Motors tab cannot reach a testable state without it.`,
    );
  }
  for (const text of bundle.missingArabic) {
    console.error(
      `   MISSING ARABIC SAFETY STRING: ${JSON.stringify(text)} - this ships as a raw i18n key on device.`,
    );
  }
